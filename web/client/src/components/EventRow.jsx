...
function EventRow({ event }) {
  ...
  // Find main 1X2 market (supports English + Spanish names)
  const mainMarket = event.markets?.find(m => {
    const n = (m.name || '').toLowerCase();
    return n.includes('resultado final') || n.includes('fulltime result') || n.includes('full time result')
      || n === 'ganador' || m.columns === '3' || m.columns === 3;
  }) || event.markets?.find(m => m.selections?.length === 3)
     || event.markets?.find(m => m.selections?.length === 2)
     || event.markets?.[0];
